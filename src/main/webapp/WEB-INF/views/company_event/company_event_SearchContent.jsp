    ...
    function writeEvent() {
		...
		<div id="right">
			...
			<div class="container">
				...
				<table width="1000" cellspacing="0" class="table">
					...
					<tr height="400">
						...
						<td style="vertical-align: top;">
							...
							<script
								src="//dapi.kakao.com/v2/maps/sdk.js?appkey=94d3da3bc59e086f2502c483efb65662&libraries=services"></script>
							<script>
				...
				<table width="1000" cellspacing="0" class="table table-hover">
					<tr>
						<c:choose>
							...
							<c:otherwise>
								<td><c:url var="company_eventSearchContentUrl"
										value="company_event_SearchContent.do">
									...
									</c:url> <a href="${company_eventSearchContentUrl }">다음글 :
										${SearchNexteventDto.event_title }</a></td>
							</c:otherwise>
						</c:choose>
					</tr>
					...
					<tr>
						<c:choose>
							...
							<c:otherwise>
								<td><c:url var="company_eventSearchContentUrll"
										value="company_event_SearchContent.do">
										<c:param name="event_idx">${SearchPreviouseventDto.event_idx }</c:param>
										<c:param name="com_idx">${dto.getCom_idx()}</c:param>
										<c:param name="eventKeyword">${eventKeyword }</c:param>
									</c:url> <a href="${company_eventSearchContentUrll }">이전글 :
										${SearchPreviouseventDto.event_title }</a></td>
							</c:otherwise>
						</c:choose>
					</tr>
				</table>
				...
				<form>
					<input id="event_writer" type="hidden" name="event_writer"
						value="${dto.getName()}"> <input id="comment_ref"
						type="hidden" name="comment_ref" value="${eventDto.comment_ref}">
					<input id="comment_sunbun" type="hidden" name="comment_sunbun"
						value="${eventDto.comment_sunbun}"> <input id="event_idx"
						type="hidden" name="event_idx" value="${eventDto.event_idx}">
					<input id="com_idx" type="hidden" name="com_idx"
						value="${dto.getCom_idx()}"> <input id=eventKeyword
						type="hidden" name="eventKeyword" value="${eventKeyword}">
					...
					<div class="form-floating mb-5" style="position: relative;">
						...
						<button id="button" class="btn btn-primary mt-2"
							...
							onclick="eventReWrite()">작성</button>
					</div>
				</form>
    ...
    function showShowResult() {
        if (XHR.readyState == 4) {
            if (XHR.status == 200) {
				...
				<div id="commentView" class="container mt-3">
